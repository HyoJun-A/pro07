...
<head>
	...
	<style>
		...
		padding: 24px; border:2px solid #eee; height:500px; overflow-y:auto; }
    </style>
</head>
...
③ 법 시행 전에 개인정보를 수집한 개인정보처리자는 기존의 수집목적 범위에도 불구하고 제1항 단서 및 제2항을 준수하기 위하여 새롭게 정보주체의 동의를 받을 목적으로 법 시행 전에 수집한 개인정보를 이용할 수 있다.
	...
	<div class="btn-group">
		<label for="ck_item3" onclick="protocol()" class="btn btn-info">전체 약관에 동의</label><br>
	</div>
	<div class="btn-group">
		<label for="ck_item3" onclick="protocol()" id="ck_item3" class="btn btn-info">다음</label><br>
	</div>
	<script>
	var ck_item1 = document.getElementById("ck_item1");
	var ck_item2 = document.getElementById("ck_item2");
	var in_btn1 = document.getElementById("ck_item3");
	in_btn1.addEventListener("click", function(){
		if(ck_item1.checked && ck_item2.checked){
			location.href = "${path1}/user/signup.do";
		} else {
